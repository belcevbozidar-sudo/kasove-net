...
export default function CartPage() {
  ...
  useEffect(() => {
    fetch("/api/suggestions")
      .then((res) => res.json())
      .then((data) => {
        setSuggestions(data.filter((p: any) => !lines.some((l) => l.productId === p.id)).slice(0, 4));
      })
  ...
  return (
    <div className="mx-auto max-w-7xl container-p py-10">
              ...
              <div key={`${line.productId}-${line.bundleProductId ?? "solo"}`} className="flex gap-4 rounded-2xl border border-border-c bg-surface p-4">
                ...
                <div className="flex flex-1 flex-col">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <Link href={`/product/${line.slug}`} className="font-heading font-semibold hover:text-accent transition-colors">
                        {line.name}
                      </Link>
      ...
      {suggestions.length > 0 && (
        <ProductRail eyebrow="Може да добавиш" title="Често купувани заедно" products={suggestions} />
      )}
    </div>
  );
}
